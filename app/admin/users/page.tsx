    ...
    <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1.5fr', gap:16 }}>
        <div className="detail-card" style={{ alignSelf:'start' }}>
          ...
          <div className="detail-row"><span className="detail-lbl">Joined</span><span className="detail-val">{new Date(selected.created_at).toLocaleDateString('en-IN')}</span></div>
          <div className="detail-row"><span className="detail-lbl">Total Orders</span><span className="detail-val" style={{ fontFamily:'var(--font-display)', fontSize:18, fontWeight:700, color:'var(--maroon)' }}>{selectedOrders.length}</span></div>
          <div style={{ marginTop:14 }}>
            <button className="btn btn-primary" style={{ width:'100%' }} onClick={() => setRoleConfirm(selected)}>
              ...
              {loading
                ...
                : users.map(u => (
                  <tr key={u.id}>
                    ...
                    <td style={{ fontSize:12, color:'var(--ink-5)' }}>{new Date(u.created_at).toLocaleDateString('en-IN')}</td>
